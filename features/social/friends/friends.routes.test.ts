...
describe("friends routes", () => {
  ...
  it("suggests friends from contact email hashes", async () => {
    ...
    const hash = createHash("sha256").update(cRow!.email).digest("hex")
    const res = await request(app)
      .post("/api/friends/suggest-from-contacts")
      .set(auth(a.token))
      .send({ emailHashes: [hash] })
    expect(res.status).toBe(200)
    expect(res.body.suggestions.some((s: any) => s.username === c.username)).toBe(true)
  })
